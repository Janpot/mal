...
replEnv.setValue('*host-language*', types.createString('ecmascript6'));
...
function extractMalCatchValue (error) {
  if (error instanceof types.MalException) {
    return error.innerValue;
  } else {
    return types.createString(error.message || 'Unknown error');
  }
}
      ...
      switch (types.getSymbolName(func)) {
        ...
        case 'try*': {
          ...
          try {
            ...
            const catchEnv = new Env(env, [ types.getSymbolName(catchArgs[0]) ], [ malException ]);
